# Day 09b    STATUS - my answer is still high by a small amount,
#               compare with Day09AM.py which works correctly.
#               I'm aborting the debug to do more important things.
...
print(f"Calculating pre-compacting checksum")
...
for dblock in datablocks[::-1]:
    ...
    if dblock[0] < freeblocks[0][0]:
        logging.info(
            f"stopping at dblock {dblock} because #1 freeblock is {freeblocks[0]}"
        )
        print(f"previous dblock {prev_dblock} previous fblock {prev_fblock}")
        break

    # preserve for end-of-loop check
    prev_dblock = dblock

    # last datablock[] first, until all have been checked
    # Try to find a free block of big enough size
...
print(f"Calculating Compacted-disk checksum")
# calculate filesystem checksum
# The datablocks list is a list of 3-tuples, each tuple is the position(ascending), the file ID and the number of blocks in that file.
csum = 0
# new checksum calculation
...
# 6725534328962 is too high
# 6363914031815 is too high
